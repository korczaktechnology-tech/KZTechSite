(() => {
  ...
  const kosModules = [
    ["ERP", "Gestão empresarial e processos administrativos."],
    ["FLOW", "Automação e orquestração de processos."],
    ["OPS", "Operação e produção."],
    ["VISION", "Gestão visual e computação visual."],
    ["CONNECT", "Integrações e contratos entre sistemas."],
    ["MOBILE", "Operação móvel."],
    ["CRM", "Relacionamento com clientes e gestão comercial."],
    ["SALES", "Processos e operações de vendas."],
    ["COMMERCE", "Operações de comércio e canais digitais."],
    ["SCM", "Gestão da cadeia de suprimentos."],
    ["STOCK", "Gestão e controle de estoque."],
    ["LOGISTICS", "Operações logísticas e movimentação."],
    ["QUALITY", "Gestão da qualidade e conformidade."],
    ["MAINTENANCE", "Manutenção e confiabilidade operacional."],
    ["PEOPLE", "Gestão de pessoas e organização."],
    ["FINANCE", "Gestão financeira e controles."],
    ["FISCAL", "Processos fiscais e obrigações."],
    ["BI", "Indicadores e inteligência de negócios."],
    ["ANALYTICS", "Dados, análises e apoio à decisão."],
  ...
  document.querySelectorAll("form[data-api]").forEach((form) =>
    form.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        if (json.data?.token)
          localStorage.setItem("kz_session", json.data.token);
        setStatus(
          form,
          form.dataset.success || "Operação concluída com sucesso.",
          "success",
        );
        form.reset();
        form.dispatchEvent(
          new CustomEvent("kz:success", { detail: json.data }),
        );
      } catch (error) {
        setStatus(
          form,
