...
async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET' && url.pathname === '/health') return response(res, 200, { success: true, data: { status: 'ok', databaseConfigured: Boolean(mongoUri) } }, requestId);
    ...
    if (req.method === 'GET' && url.pathname === '/api/v1/health/database') {
      await databaseRef.command({ ping: 1 }); return response(res, 200, { success: true, data: { status: 'ok', database: dbName } }, requestId);
    }
